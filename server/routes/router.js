    ...
    const deletesession = async (AdminSessionId) => {
        try {
            ...
            studentsessionid.forEach((sessionid) => {
                ...
                const screenfolderPath = path.join(__dirname, `../${sessionid}/screenshare`);
                const webcamfolderPath = path.join(__dirname, `../${sessionid}/webcam`);
                if (fs.existsSync(folderPath)) {
                    fs.readdirSync(screenfolderPath).forEach((file) => {
                        const curPath = path.join(screenfolderPath, file);
                    ...
                    fs.readdirSync(webcamfolderPath).forEach((file) => {
                        const curPath = path.join(webcamfolderPath, file);
                        if (fs.lstatSync(curPath).isDirectory()) {
                            deleteFolderRecursive(curPath);
                        } else {
                            fs.unlinkSync(curPath);
                    ...
                    fs.rmdirSync(screenfolderPath);
                    fs.rmdirSync(webcamfolderPath);
...
router.get("/deletestudent/:id/:studentid", async (req, res) => {
    ...
    const deletesession = async (AdminSessionId, _id) => {
        try {
            ...
            const screenfolderPath = path.join(__dirname, `../${_id}/screenshare`);
            const webcamfolderPath = path.join(__dirname, `../${_id}/webcam`);
            if (fs.existsSync(folderPath)) {
                fs.readdirSync(screenfolderPath).forEach((file) => {
                    const curPath = path.join(screenfolderPath, file);
                    if (fs.lstatSync(curPath).isDirectory()) {
